...
const CoachReport: React.FC<CoachReportProps> = ({ profile, matches }) => {
    ...
    const handleGenerate = async () => {
        if (monthMatches.length === 0) { showToast('No matches in selected month', 'error'); return; }
        if (selectedPersona === 'custom' && (!customName || !customStyle)) { showToast('Please enter coach name and style', 'error'); return; }
        setIsLoading(true);
        try {
    ...
    const handleDownloadLetter = async () => {
        ...
        try {
            ...
            const canvas = await html2canvas(reportRef.current, { useCORS: true, scale: 2, backgroundColor: null, logging: false });
    ...
    const handleImageError = (persona: string) => {
        if (!avatarErrors[persona]) setAvatarErrors(prev => ({ ...prev, [persona]: true }));
    };
    ...
    const triggerUpload = () => {
        if (selectedPersona === 'custom') setTimeout(() => fileInputRef.current?.click(), 100);
    };
    ...
    const renderAvatar = (persona: CoachPersona) => {
        if (persona === 'custom') {
            ...
            if (customSrc) return <img src={customSrc} alt="Custom Coach" className="w-full h-full object-cover" />;
            return <div className="w-full h-full flex items-center justify-center bg-slate-100 text-slate-400"><i className="fas fa-user-secret text-2xl"></i></div>;
        }
        const url = DEFAULT_AVATAR_URLS[persona as keyof typeof DEFAULT_AVATAR_URLS];
        const hasError = avatarErrors[persona];
        if (!url || hasError) {
            let iconClass = 'fa-user-tie', bgClass = 'bg-slate-200 text-slate-400';
            ...
            return <div className={`w-full h-full flex items-center justify-center ${bgClass}`}><i className={`fas ${iconClass} text-2xl`}></i></div>;
        }
        return <img src={url} alt={persona} className="w-full h-full object-cover" onError={() => handleImageError(persona)} loading="lazy" />;
    };
    ...
    return (
        <div className="pb-24 animate-fade-in bg-slate-50 min-h-screen">
            ...
            <div className="bg-slate-900 text-white p-4 sticky top-0 z-20 shadow-md">
                ...
                <div className="flex bg-white/10 rounded-xl p-1 gap-1">
                    <button onClick={() => setActiveTab('coach')}
                        className={`flex-1 py-2 rounded-lg text-xs font-black transition-all flex items-center justify-center gap-1.5 ${activeTab === 'coach' ? 'bg-white text-slate-900' : 'text-white/70 hover:text-white'}`}>
                        <i className="fas fa-magic text-xs" />
                        {language === 'zh' ? '教練報告' : 'Coach Report'}
                    </button>
                    <button onClick={() => setActiveTab('growth')}
                        className={`flex-1 py-2 rounded-lg text-xs font-black transition-all flex items-center justify-center gap-1.5 ${activeTab === 'growth' ? 'bg-white text-slate-900' : 'text-white/70 hover:text-white'}`}>
            ...
            <div className="p-4 space-y-6 max-w-lg mx-auto">
                ...
                {/* ── Growth Story Tab ── */}
                {activeTab === 'growth' && (
                    <GrowthStory profile={profile} matches={matches} />
                )}
                        ...
                        <div>
                            ...
                            <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
                                {(['motivator', 'tactician', 'wisdom', 'custom'] as CoachPersona[]).map(persona => {
                                    ...
                                    return (
                                        <button key={persona} onClick={() => setSelectedPersona(persona)}
        ...
        <button onClick={handleGenerate} disabled={isLoading || monthMatches.length === 0}
                    ...
                    {currentReport && (
                        <div className="animate-fade-in-up">
                            ...
                            <button onClick={handleDownloadLetter} disabled={isDownloading}
